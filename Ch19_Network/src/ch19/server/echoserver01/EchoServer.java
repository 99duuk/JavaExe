...
public class EchoServer {
	public static void main(String[] args) {
		...
		try {
			...
			PrintWriter pw = new PrintWriter(new OutputStreamWriter(out)); // 전송 --> 필터 통해 래핑
			BufferedReader br = new BufferedReader(new InputStreamReader(in)); // 수신

			// 클라이언트와 통신 시작
			String line;
			while (true) {
				line = br.readLine();	// 내부 버퍼로부터 '\n'까지 읽어들여라. ( 개행문자, 버퍼에 있는 문자 받아 넣어줌)
				if (line == null) // 통신이 끊어졌다. 상대방이 종료했다.
					break;
				System.out.println("클라이언트로부터 수신 : " + line); 		// 널값 아니면 line 받아 출력
				pw.println(line); // 데이터 + '\n';						
				pw.flush(); // 버퍼에 남아있는 데이터 즉시 전송해		//클라이언트한테 바로 ㅅ쏴줄 수 있도록 ㅇㅇ
			}
