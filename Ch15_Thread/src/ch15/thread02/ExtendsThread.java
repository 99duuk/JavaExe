...
/*
 [스레드 생성 요청 2가지 방법]
	vm 내부에 스레드 리소스 존재,
	우리는 객체의 참조 변수로 스레드의 동작 제어 
	1) Thread 클래스의 상속받는 클래스 구현
	2) Runnable 인터페이스 상속 받는 클래스 구현
		→ 이미 다른 클래스의 상속 받은 클래스에서 스레드를 동작시키려면
			Thread 클래스를 더 이상 상속시킬 수 없다. 
			Because java 단일상속 ㅇㅇ
			So 이미 다른 클래스 상속받은 클래스에서 스레드 동작시키기 위해 
			Runnable 인터페이스를 상속받는 방법을 제시
	
	모두 run()에 스레드가 해야할 일을 구현한다. 
	start() 호출하면 스레드가 동작 시작한다.
...
public class ExtendsThread  {
	public static void main(String[] args) {
		...
		//스레드를 동작(run()을 호출)
		System.out.println("t1 스레드 동작"); t1.start();
		System.out.println("t2 스레드 동작"); t2.start();
		System.out.println("t3 스레드 동작"); t3.start();
		System.out.println("t4 스레드 동작"); t4.start();
